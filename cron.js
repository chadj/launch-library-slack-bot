...
async function poll(event, context, callback) {
    ...
    if (d_resp.Count > 0) {
        ...
        const eventManifest = await request({
            method: 'GET',
            uri: 'https://ll.thespacedevs.com/2.0.0/event/upcoming/',
            qs: {
                format: 'json',
                limit: '20'
            },
        ...
        for (const event of eventManifest.results) {
            if (!event.date) {
                return;
            }

            const event_date = new Date(event.date);
            if (event_date >= cutoff_start && event_date < cutoff_end) {
                // Event today
                const event_start = moment(event_date).tz(settings.SLACK_TZ_NAME);
                const event_start_fmt = event_start.format("M/D h:mm A zz");
                const event_start_unx = event_date.getTime() / 1000;

                const name_extra = [];
                if(event.news_url) {
                    name_extra.push(`<${event.news_url}|news>`);
                }
                if (event.program.length > 0) {
                    for (const p of event.program) {
                        if(p.wiki_url) {
                            name_extra.push(`<${p.wiki_url}|wiki>`);
                        }
                        if(p.info_url) {
                            name_extra.push(`<${p.info_url}|more>`);
                        }
                    }
                }
                ...
                let message = `*${event.name}*${name_extra_fmt}\n`;
                message += `Start: <!date^${event_start_unx}^{date_short_pretty} at {time}|${event_start_fmt}>\n`;

                if(event.type && event.type.name) {
                    message += `Type: ${event.type.name}\n`;
                }
                if(event.location) {
                ...
                if (event.video_url) {
                    actions.push({
                        "type": "button",
                        "text": "Watch Live",
                        "fallback": `Watch it live at ${event.video_url}`,
                        "url": event.video_url
                    });
                }
                ...
                attachments.push({
                    "color": "#005883",
                    "fallback": "Event Actions",
                    "callback_id": `event-${event.id}`,
                    "actions": actions
                });
